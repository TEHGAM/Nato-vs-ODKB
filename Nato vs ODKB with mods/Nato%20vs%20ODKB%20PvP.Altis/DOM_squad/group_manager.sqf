...
GVAR(actions_addBack) = {	
	player addAction [
		"<t color='#cccccc'><img image='\A3\ui_f\data\gui\rsc\rscdisplayarcademap\icon_sidebar_show.paa' size='0.7' /> ... Назад</t>",
...
GVAR(join) = {
	if(([group _THIS(0)] call GVAR(options_getJoin))!=JOIN_FREE) exitWith {
		format["Отряд %1 (командир отряда %2) закрыт для вступления", name _THIS(0), name leader _THIS(0)] call GVAR(msg);
	};
	[
	...
	format["Вы вступили в отряд %1 под командованием %2", name _THIS(0), name leader _THIS(0)] call GVAR(msg);	
...
GVAR(invite) = {
	...
	if(!(
		...
		"Вы не можете отправлять приглашения о вступлении в отряд" call GVAR(msg);
	};
	format["Вы пригласили %1 в отряд", name _THIS(0)] call GVAR(msg);
	[
		format["%1 пригласил %2 в ваш отряд", name player, name _THIS(0)],
		QGVAR(msg),
		[(units group player)-[player]] call GVAR(playersOnly)	
	] spawn BIS_fnc_MP;
	[
...
GVAR(invited) = {
	if(_THIS(0) in units _THIS(1)) then {
		format["%1 приглашает вас вступить в его отряд (командир отряда %2)", name _THIS(0), name leader _THIS(1)] call GVAR(msg);
		{
...
GVAR(invite_accepted) = {	
	format["Приглашение вступить в отряд от игрока %1 (командир отряда %2) принято", name _THIS(0), name leader _THIS(0)] call GVAR(msg),
	[
		format["%1 принял приглашение вступить в отряд", name player],
		QGVAR(msg),
		[[_THIS(0)]] call GVAR(playersOnly)		
	] spawn BIS_fnc_MP;
	[
		format["%1 вступает в отряд. Приглашение направил %2", name player, name _THIS(0)],
		QGVAR(msg),
...
GVAR(invite_declined) = {	
	format["Отклонено приглашение в отряд от игрока %1 (командир отряда %2)", name _THIS(0), name leader _THIS(0)] call GVAR(msg),
	[
		format["%1 отклонил ваше приглашение в отряд", name player],
		QGVAR(msg),
		[[_THIS(0)]] call GVAR(playersOnly)		
	] spawn BIS_fnc_MP;
	GVAR(invites) set[_THIS(1), 0];
...
GVAR(request) = {
	...
	if(!(
		...
		format["Вы больше не можете посылать запрос на вступление в отряд %1 (командир отряда %2)", name _THIS(1), name leader _THIS(1)] call GVAR(msg);
	};				
	format["Вы отправили запрос на вступление в отряд %1 (командир отряда %2)", name _THIS(1), name leader _THIS(1)] call GVAR(msg);
	[
		[
			_THIS(0),
			group _THIS(1)
...
GVAR(requested) = {
	if(player in units _THIS(1)) then {
		format["%1 направил вам запрос о вступлении в ваш отряд", name _THIS(0), name leader _THIS(1)] call GVAR(msg);
		{
			if((_x select 1)==_THIS(0) && (_x select 2)==_THIS(1)) then {
...
GVAR(request_accepted) = {
	format["Запрос на вступление в отряд игрока %1 одобрен", name _THIS(0)] call GVAR(msg),
	[
		format["%1 (командир отряда %2) одобрил ваш запрос на вступление в отряд", name player, name leader player],
		QGVAR(msg),
		[[_THIS(0)]] call GVAR(playersOnly)		
	] spawn BIS_fnc_MP;
	[
		format["%1 присоединился к отряду (одобрено %2)", name _THIS(0), name player],
		QGVAR(msg),
		[(units group player)-[player]] call GVAR(playersOnly)		
	] spawn BIS_fnc_MP;		
...
GVAR(request_declined) = {
	format["Запрос на вступление в отряд игрока %1 отклонен", name _THIS(0)] call GVAR(msg),
	[
		format["%1 (командир отряда %2) ваш запрос на вступление в отряд", name player, name leader player],
		QGVAR(msg),
		[[_THIS(0)]] call GVAR(playersOnly)		
	] spawn BIS_fnc_MP;	
	GVAR(requests) set[_THIS(1), 0];
...
GVAR(takeLeaderShip) = {
	...
	if(!([player] call GVAR(canTakeLeadership))) exitWith {
		"Вы не можете стать командиром отряда" call GVAR(msg); 
		[] call GVAR(menu_main);
	};
	"Вы приняли командование отрядом" call GVAR(msg);	
	[
		format["%1 принял командование отрядом", name player],
		QGVAR(msg),
...
GVAR(takeLeaderShip_remote) = {
	if(group _THIS(0) == group player) then {
		if(isPlayer leader player) then {
			format["%1 принял от вас командование отрядом", name _THIS(0)] call GVAR(msg);
		};
...
GVAR(menu_giveLeaderShip) = {
	...
	if(leader player!=player) exitWith {
		"Вы не являетесь командиром" call GVAR(msg); 
		[] call GVAR(menu_main);
	};
	[] call GVAR(actions_remove);
	{
		PUSH_START(GVAR(actions_ids))
			player addAction [
				format["<t color='#0099ee'><img image='\A3\ui_f\data\gui\Rsc\RscDisplayConfigViewer\bookmark_gs.paa' size='0.7' /> Отдать командование игроку %1</t>", name _x],
...
GVAR(giveLeaderShip) = {
	format["Отдать командование отрядом %1", name _THIS(0)] call GVAR(msg);
	[
		format["%1 передал командование отрядом %2", name _THIS(0), name player],
		QGVAR(msg),
		[(units group _THIS(0))-[_THIS(0), player]] call GVAR(playersOnly)		
	] spawn BIS_fnc_MP;
	[
		format["%1 передал вам командование отрядом", name player],
		QGVAR(msg),
		[[_THIS(0)]] call GVAR(playersOnly)		
	] spawn BIS_fnc_MP;			
...
GVAR(menu_squadOptions) = {
	...
	_join = [group player] call GVAR(options_getJoin);
	{
		...
		PUSH_END		
	} forEach ["Все игроки могут присоединять в отряд","Члены отряда могут приглашать других игроков","Приглашать других игроков может только командир отряда","Запретить приглашения"];	
	_accept = [group player] call GVAR(options_getAccept);  		
	{
		...
		PUSH_END		
	} forEach ["Члены отряда могут разрешать вступать в отряд другим игрокам","Только командир отряда может разрешать вступать в отряд другим игрокам","Отменить запросы на вступление в отряд"];	
	[] call GVAR(actions_addBack);	
};
...
GVAR(menu_kickSquadMember) = {
	...
	[] call GVAR(actions_remove);
	{
		PUSH_START(GVAR(actions_ids)) 
			player addAction [
				format["<t color='#ff8822'><img image='\A3\ui_f\data\gui\rsc\rscdisplayarcademap\top_close_gs.paa' size='0.7' /> Исключить игрока %1</t>", name _x],
...
GVAR(kickSquadMember) = {
	...
	format["Вы исключили из отряда игрока %1", name _THIS(0)] call GVAR(msg); 				
	[
		format["%1 исключен из отряда игроком %2", name _THIS(0), name player],
		QGVAR(msg),
...
GVAR(kickSquadMember_remote) = {
	if(isPlayer _THIS(1)) then {	
		format["Вы были исключены из отряда игроком %1", name _THIS(0)] call GVAR(msg);
	};		
...
GVAR(menu_main) = {
	...
	if(leader player == player) then {
		if(count units group player > 1)then {
			PUSH_START(GVAR(actions_ids))
				player addAction [
					"<t color='#0099ee'><img image='\A3\ui_f\data\gui\Rsc\RscDisplayConfigViewer\bookmark_gs.paa' size='0.7' /> Отдать командование игроку ...</t>",
			...
			PUSH_START(GVAR(actions_ids))
				player addAction [
					"<t color='#ff8822'><img image='\A3\ui_f\data\gui\rsc\rscdisplayarcademap\top_close_gs.paa' size='0.7' /> Исключить из отряда игрока ...</t>",
		...
		PUSH_START(GVAR(actions_ids))
			player addAction [
				"<t color='#0088ee'><img image='\A3\ui_f\data\gui\rsc\rscdisplayarcademap\icon_config_ca.paa' size='0.7' /> Настройки отряда ...</t>",
	...
	GVAR(invites) = GVAR(invites) - [0];
	{
		...
		if(_unit1 in units (_x select 2) && (_x select 0) + TIMEOUT > time) then {						
			PUSH_START(GVAR(actions_ids))
				player addAction [
					format["<t color='#00cc00'><img image='\A3\ui_f\data\gui\rsc\rscdisplayarcademap\icon_continue_ca.paa' size='0.7' /> Принять приглашение в отряд от игрока %1 (командир отряда %2)</t>", name _unit1, name leader _unit1],
			...
			PUSH_START(GVAR(actions_ids))
				player addAction [
					format["<t color='#ff1111'><img image='\A3\ui_f\data\gui\rsc\rscdisplayarcademap\top_close_gs.paa' size='0.7' /> Отклонить приглашение в отряд от игрока %1 (командир отряда %2)</t>", name _unit1, name leader _unit1],
	...
	GVAR(requests) = GVAR(requests) - [0];
	{
	  	...
	  	if(player in units (_x select 2) && (_x select 0) + TIMEOUT > time) then {						
			PUSH_START(GVAR(actions_ids))
				player addAction [
					format["<t color='#00cc00'><img image='\A3\ui_f\data\gui\rsc\rscdisplayarcademap\icon_continue_ca.paa' size='0.7' /> Одобрить запрост на вступление в отряд от игрока %1</t>", name _unit1],
			...
			PUSH_START(GVAR(actions_ids))
				player addAction [
					format["<t color='#ff1111'><img image='\A3\ui_f\data\gui\rsc\rscdisplayarcademap\top_close_gs.paa' size='0.7' /> Отклонить запрост на вступление в отряд от игрока %1</t>", name _unit1],
		  ...
		  && alive _x && !(vehicle _x in allUnitsUav) 		  
			...
			if(_myJoin!=JOIN_DISABLED && isPlayer _x) then {
				if(
					...
					PUSH_START(GVAR(actions_ids))
						player addAction [
							format["<t color='#ffcc66'><img image='\A3\ui_f\data\gui\rsc\rscdisplayarcademap\icon_toolbox_units_ca.paa' size='0.7' /> Пригласить %1 в ваш отряд</t>", name _x],
			...
			if(!(group _x in _groupsDone)) then {
				...
				if(_join==JOIN_FREE) then {
					PUSH_START(GVAR(actions_ids))
						player addAction [
							format["<t color='#ffcc66'><img image='\A3\ui_f\data\gui\rsc\rscdisplayarcademap\icon_toolbox_units_ca.paa' size='0.7' /> Присоединиться к отряду %1 (командир отряда %2)</t>", name _x, name leader _x],
					...
					if(
						...
						PUSH_START(GVAR(actions_ids)) 
							player addAction [
								format["<t color='#ffcc66'><img image='\A3\ui_f\data\gui\rsc\rscdisplayarcademap\icon_toolbox_units_ca.paa' size='0.7' /> Отправить запрос на вступление в отряд %1 (командир отряда %2)</t>", name _x, name leader _x],
